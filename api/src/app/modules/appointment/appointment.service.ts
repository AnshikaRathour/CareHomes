...
const createAppointment = async (payload: any): Promise<Appointments | null | any> => {
    ...
    const result = await prisma.$transaction(async (tx: { appointments: { findFirst: (arg0: { orderBy: { createdAt: string; }; take: number; }) => any; create: (arg0: { data: any; include: { doctor: boolean; patient: boolean; }; }) => any; }; payment: { create: (arg0: { data: { appointmentId: any; bookingFee: number; paymentMethod: any; paymentType: any; vat: number; DoctorFee: number; totalAmount: number; }; }) => any; }; }) => {
...
const createAppointmentByUnAuthenticateUser = async (payload: any): Promise<Appointments | null> => {
    ...
    const result = await prisma.$transaction(async (tx: { appointments: { findFirst: (arg0: { orderBy: { createdAt: string; }; take: number; }) => any; create: (arg0: { data: any; }) => any; }; payment: { create: (arg0: { data: { appointmentId: any; bookingFee: number; paymentMethod: any; paymentType: any; vat: number; DoctorFee: number; totalAmount: number; }; }) => any; }; }) => {
        const previousAppointment = await tx.appointments.findFirst({
            orderBy: { createdAt: 'desc' },
            take: 1
        });
...
const getDoctorPatients = async (user: any): Promise<Patient[]> => {
    ...
    const patientIds = patients.map((appointment: { patientId: any; }) => appointment.patientId);
